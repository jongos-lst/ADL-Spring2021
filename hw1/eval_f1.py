...
def main(args):
    ...
    eval_loader = DataLoader(dataset, batch_size=args.batch_size, collate_fn = dataset.collate_fn, shuffle=False)
        ...
        for i in range(len(batch['tokens'])):
            df2 = pd.DataFrame(
                [[batch['id'][i], ' '.join([ dataset.idx2label(int(test_pred[i][j])) for j in range(batch['length'][i])]) ]],
                   columns=['id', 'tags'])
            predicts.append([ dataset.idx2label(int(test_pred[i][j])) for j in range(batch['length'][i])])
            ans.append(batch['tags_raw'][i])
            df = pd.concat([df, df2]) 
    # TODO: write prediction to file (args.pred_file)
    #df.to_csv("./slot_test_pred.csv", index=False)
    
    #print(ans)
    #print(predicts)

    #print(f1_score(ans, predicts))
    print(classification_report(ans, predicts))
